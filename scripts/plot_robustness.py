...
def main():
    ...
    # Standard pi-pulse: A * sigma * sqrt(2pi) = pi
    # Let's say sigma = duration/4
    sigma_std = duration / 4.0
    A_std = np.pi / (sigma_std * np.sqrt(2 * np.pi))
    params_std = np.array([A_std, duration/2, sigma_std, 0.0, 0.0])
    ...
    # Sweep Detuning
    detunings = np.linspace(-20e6, 20e6, 41) # Hz
    ...
    print("Sweeping detuning +/- 20.0 MHz...")
